...
describe('Auth page', () => {
    ...
    it('user can switch between signIn and signUp modes', () => {
        ...
        cy.get('*[class^="AuthForm_toggle__"]')
          .click()
        cy.get('h1')
    ...
    it('user can register', () => {
        ...
        cy.get('#email')
          .type(email)
        cy.get('#password')
          .type('passwordtest123')
        cy.get('form')
          .submit()
        cy.url().should('not.include','/auth')
        cy.get('h1')
          .should('have.text','Welcome on Board!')
    })
    ...
    it('user can access to his profile and change password', () => {
        ...
        cy.get('h1')
          .should('have.text','Your User Profile')
        cy.get('#new-password')
         .type('newpasswordtest123')
        cy.intercept('**/accounts:update?key=*').as('updatePassword')
        cy.get('*[class^="ProfileForm_form__"]')
          .submit()
    ...
    it('user entered a short password in his profile and receive a response error in a modal', () => {
        ...
        cy.get('.p-dialog')
          .should('exist')
          .find('strong')
          .should('have.text', 'WEAK_PASSWORD : Password should be at least 6 characters')
        cy.get('.p-dialog-header-close-icon')
          .click()
        cy.get('*[class^="MainNavigation_logo__"]')
          .click()
    })
  });